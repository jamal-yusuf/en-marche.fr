...
class TransactionCallbackHandler
{
    ...
    public function __construct(UrlGeneratorInterface $router, ObjectManager $entityManager, MailjetService $mailjet, DonationRequestHelper $donationRequestHelper)
    {
        $this->router = $router;
    ...
    private function createRedirectResponseForDonation(Donation $donation): Response
    {
        return new RedirectResponse($this->router->generate(
            'donation_result',
            $this->donationRequestHelper->createCallbackStatus($donation)
        ));
    }
}
